...
FeatureExtractor::FeatureExtractor() 
{
  ...
  image_pc_sync_ = std::make_shared<message_filters::Synchronizer<ImageLidarSyncPolicy>>(ImageLidarSyncPolicy(queue_rate_), *image_sub_, *pc_sub_);
...
void FeatureExtractor::optimise(const RunOptimiseGoalConstPtr& goal, actionlib::SimpleActionServer<RunOptimiseAction>* as) 
{
...
auto FeatureExtractor::chessboardProjection(const std::vector<cv::Point2d>& corners, const cv_bridge::CvImagePtr& cv_ptr) 
{
  ...
  board_corners_3d.push_back(cv::Point3d((board_width_ - i_params.cb_translation_error.x) / 2.0, (board_height_ - i_params.cb_translation_error.y) / 2.0, 0.0));
...
std::pair<pcl::ModelCoefficients, pcl::ModelCoefficients> FeatureExtractor::findEdges(const PointCloud::Ptr& edge_pair_cloud) 
{
...
void FeatureExtractor::extractRegionOfInterest(const sensor_msgs::Image::ConstPtr& image, const PointCloud::ConstPtr& pointcloud) 
{
  ...
  if (flag == Optimise::Request::CAPTURE) {
    ...
    if (num_of_pc_frames_ == frames_to_capture_) 
    {
      ...
      else  // For every other sample get the dimensions and compare to the first one
      {
        ...
        cv::Point3d avg_camera_c0 = cv::Point3d(sum_camera_c0.x / samples_.size(),
                                                sum_camera_c0.y / samples_.size(),
                                                sum_camera_c0.z / samples_.size());
        cv::Point3d avg_camera_c1 = cv::Point3d(sum_camera_c0.x / samples_.size(),
                                                sum_camera_c0.y / samples_.size(),
                                                sum_camera_c0.z / samples_.size());
        cv::Point3d avg_camera_c2 = cv::Point3d(sum_camera_c0.x / samples_.size(),
                                                sum_camera_c0.y / samples_.size(),
                                                sum_camera_c0.z / samples_.size());
        cv::Point3d avg_camera_c3 = cv::Point3d(sum_camera_c0.x / samples_.size(),
                                                sum_camera_c0.y / samples_.size(),
                                                sum_camera_c0.z / samples_.size());

        cv::Point3d avg_lidar_c0 = cv::Point3d(sum_lidar_c0.x / samples_.size(), 
                                               sum_lidar_c0.y / samples_.size(),
                                               sum_lidar_c0.z / samples_.size());

        cv::Point3d avg_lidar_c1 = cv::Point3d(sum_lidar_c1.x / samples_.size(), 
                                               sum_lidar_c1.y / samples_.size(),
                                               sum_lidar_c1.z / samples_.size());

        cv::Point3d avg_lidar_c2 = cv::Point3d(sum_lidar_c2.x / samples_.size(), 
                                               sum_lidar_c2.y / samples_.size(),
                                               sum_lidar_c2.z / samples_.size());

        cv::Point3d avg_lidar_c3 = cv::Point3d(sum_lidar_c3.x / samples_.size(), 
                                               sum_lidar_c3.y / samples_.size(),
        ...
        sample.camera_centre = cv::Point3d(sum_camera_centre.x / samples_.size(),
                                           sum_camera_centre.y / samples_.size(),
                                           sum_camera_centre.z / samples_.size());

        sample.camera_normal = cv::Point3d(sum_camera_normal.x / samples_.size(),
        ...
        printf("Board angles     = %5.2f,%5.2f,%5.2f,%5.2f degrees\n", sample.angles[0], sample.angles[1], sample.angles[2], sample.angles[3]);
        ...
        printf("Avg Lidar Normal = %6.2f, %6.2f, %6.2f \n", sample.lidar_normal.x, sample.lidar_normal.y, sample.lidar_normal.z);
        printf("Avg Lidar Centre = %6.2f, %6.2f, %6.2f \n", sample.lidar_centre.x, sample.lidar_centre.y, sample.lidar_centre.z);
        printf("Std Dev Lidar Normal = %6.2f, %6.2f, %6.2f \n", sample.lidar_normal_stddev.x, sample.lidar_normal_stddev.y, sample.lidar_normal_stddev.z);
        printf("Std Dev Lidar Centre = %6.2f, %6.2f, %6.2f \n\n", sample.lidar_centre_stddev.x, sample.lidar_centre_stddev.y, sample.lidar_centre_stddev.z);

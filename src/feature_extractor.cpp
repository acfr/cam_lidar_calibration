...
FeatureExtractor::FeatureExtractor() {
  ...
  server = boost::make_shared<
      dynamic_reconfigure::Server<cam_lidar_calibration::boundsConfig>>(pnh);
  dynamic_reconfigure::Server<cam_lidar_calibration::boundsConfig>::CallbackType
      f;
  ...
  image_sub_ = std::make_shared<image_sub_type>(
      private_nh, i_params.camera_topic, queue_rate_);
  pc_sub_ = std::make_shared<pc_sub_type>(private_nh, i_params.lidar_topic,
                                          queue_rate_);

  image_pc_sync_ =
      std::make_shared<message_filters::Synchronizer<ImageLidarSyncPolicy>>(
          ImageLidarSyncPolicy(queue_rate_), *image_sub_, *pc_sub_);
  image_pc_sync_->registerCallback(
      boost::bind(&FeatureExtractor::extractRegionOfInterest, this, _1, _2));
  ...
  experimental_region_pub_ =
      private_nh.advertise<PointCloud>("experimental_region", 10);
  optimise_service_ = public_nh.advertiseService(
      "optimiser", &FeatureExtractor::serviceCB, this);
  samples_pub_ = private_nh.advertise<visualization_msgs::MarkerArray>(
      "collected_samples", 0);
  ...
  camera_info_sub_ = public_nh.subscribe(
      i_params.camera_info, 20, &FeatureExtractor::callback_camerainfo, this);
  ...
  if (import_samples) {
    ...
    std::string data_dir =
        ros::package::getPath("cam_lidar_calibration") + "/data";
...
void FeatureExtractor::callback_camerainfo(
  ...
  if (msg->distortion_model == "equidistant") {
    ...
    // Pinhole
  } else if (msg->distortion_model == "rational_polynomial" or
             msg->distortion_model == "plumb_bob") {
    i_params.fisheye_model = false;
  } else {
    ROS_FATAL_STREAM("Camera model " << msg->distortion_model
                                     << " not supported");
  }
  valid_camera_info = true;
}

bool FeatureExtractor::serviceCB(Optimise::Request& req,
                                 Optimise::Response& res) {
...
void FeatureExtractor::optimise(
  ...
  if (import_samples) {
    ...
    save_samples.open(savesamplespath,
                      std::ios_base::out | std::ios_base::trunc);

    for (OptimisationSample s : optimiser_->samples) {
      save_samples << s.camera_centre.x << "," << s.camera_centre.y << ","
                   << s.camera_centre.z << "\n";
      save_samples << s.camera_normal.x << "," << s.camera_normal.y << ","
                   << s.camera_normal.z << "\n";
      for (auto cc : s.camera_corners) {
        save_samples << cc.x << "," << cc.y << "," << cc.z << "\n";
      }
      save_samples << s.lidar_centre.x << "," << s.lidar_centre.y << ","
                   << s.lidar_centre.z << "\n";
      save_samples << s.lidar_normal.x << "," << s.lidar_normal.y << ","
                   << s.lidar_normal.z << "\n";
    ...
    for (int j = 0; j < 19600; j++) {
      for (int i = 0; i < 3; i++) {
        ...
        auto it = std::find_if(set.begin(), set.end(),
                               [&s_num](const OptimisationSample& obj) {
        ...
        while (it != set.end()) {
          ...
          it = std::find_if(set.begin(), set.end(),
                            [&new_sample](const OptimisationSample& obj) {
  ...
  for (std::size_t i = 0; i < optimiser_->sets.size(); i++) {
    ...
    for (auto& sample : optimiser_->sets[i]) {
      float err_dim = abs(sample.widths[0] - board_width_) +
                      abs(sample.widths[1] - board_width_) +
                      abs(sample.heights[0] - board_height_) +
                      abs(sample.heights[1] - board_height_);
    ...
    float b_avg =
        std::accumulate(std::begin(be), std::end(be), 0.0) / be.size();

    // Commutative property holds for AA^{-1} = A^{-1}A = I (in the case of a
    // well conditioned matrix)
    float cn_cond_fro = cv::norm(camera_normals_, cv::NORM_L2) *
                        cv::norm(camera_normals_.inv(), cv::NORM_L2);
    float ln_cond_fro = cv::norm(lidar_normals_, cv::NORM_L2) *
                        cv::norm(lidar_normals_.inv(), cv::NORM_L2);
  ...
  ROS_INFO_STREAM("voq range: " << calib_list.front().voq << "-"
                                << calib_list.back().voq);
  ROS_INFO_STREAM("Number of assessed sets: " << num_assessed);
  ROS_INFO_STREAM(optimiser_->top_sets.size()
                  << " selected sets for optimisation");
  ...
  printf(
      " Computing calibration results (roll,pitch,yaw,x,y,z) for each of the "
      "%d lowest voq sets\n",
  ...
  for (std::size_t i = 0; i < optimiser_->top_sets.size(); i++) {
    ...
    success = optimiser_->optimise(opt_result, optimiser_->top_sets[i],
                                   i_params.cameramat, i_params.distcoeff);

    // Save extrinsic params to csv for post processing
    if (success) {
      output_csv << opt_result.rot.roll << "," << opt_result.rot.pitch << ","
                 << opt_result.rot.yaw << "," << opt_result.x / 1000.0 << ","
                 << opt_result.y / 1000.0 << "," << opt_result.z / 1000.0
                 << "\n";
    }
    printf("| t: %.3fs\n", timer_set.toc());
    output_csv.close();
...
void FeatureExtractor::boundsCB(cam_lidar_calibration::boundsConfig& config,
                                uint32_t level) {
  // Read the values corresponding to the motion of slider bars in reconfigure
  // gui
  bounds_ = config;
  ROS_INFO("Reconfigure Request: %lf %lf %lf %lf %lf %lf %d %lf", config.x_min,
           config.x_max, config.y_min, config.y_max, config.z_min, config.z_max,
           config.k, config.z, config.voxel_res);
}
...
void FeatureExtractor::passthrough(const PointCloud::ConstPtr& input_pc,
                                   PointCloud::Ptr& output_pc) {
...
auto FeatureExtractor::chessboardProjection(
  ...
  // chessboard
  cv::Point3d chessboard_bleft_corner(
      (i_params.chessboard_pattern_size.width - 1) * i_params.square_length / 2,
      (i_params.chessboard_pattern_size.height - 1) * i_params.square_length /
          2,
      0);
  ...
  for (int y = 0; y < i_params.chessboard_pattern_size.height; y++) {
    for (int x = 0; x < i_params.chessboard_pattern_size.width; x++) {
      corners_3d.push_back(cv::Point3d(x, y, 0) * i_params.square_length -
                           chessboard_bleft_corner);
    }
  }
  ...
  board_corners_3d.push_back(cv::Point3d(
      (board_width_ - i_params.cb_translation_error.x) / 2.0,
  ...
  board_corners_3d.push_back(cv::Point3d(-i_params.cb_translation_error.x / 2.0,
                                         -i_params.cb_translation_error.y / 2.0,
                                         0.0));
  ...
  if (valid_camera_info) {
    if (i_params.fisheye_model) {
      ...
      cv::fisheye::undistortPoints(corners, corners_undistorted,
                                   i_params.cameramat, i_params.distcoeff,
                                   i_params.cameramat);
      cv::solvePnP(corners_3d, corners_undistorted, i_params.cameramat,
                   cv::noArray(), rvec, tvec);
      cv::fisheye::projectPoints(corners_3d, inner_cbcorner_pixels, rvec, tvec,
                                 i_params.cameramat, i_params.distcoeff);
      cv::fisheye::projectPoints(board_corners_3d, board_image_pixels, rvec,
                                 tvec, i_params.cameramat, i_params.distcoeff);
    } else {
      // Pinhole model
      cv::solvePnP(corners_3d, corners, i_params.cameramat, i_params.distcoeff,
                   rvec, tvec);
      cv::projectPoints(corners_3d, rvec, tvec, i_params.cameramat,
                        i_params.distcoeff, inner_cbcorner_pixels);
      cv::projectPoints(board_corners_3d, rvec, tvec, i_params.cameramat,
                        i_params.distcoeff, board_image_pixels);
    }
  } else {
    ROS_FATAL(
        "No msgs from /camera_info - check camera_info topic in "
        "cfg/params.yaml is correct and is being published");
  }

  for (std::size_t i = 0; i < board_image_pixels.size(); i++) {
    if (i == 0) {
      cv::circle(cv_ptr->image, board_image_pixels[i], 4, CV_RGB(255, 0, 0),
                 -1);
    } else if (i == 1) {
      cv::circle(cv_ptr->image, board_image_pixels[i], 4, CV_RGB(0, 255, 0),
                 -1);
    } else if (i == 2) {
      cv::circle(cv_ptr->image, board_image_pixels[i], 4, CV_RGB(0, 0, 255),
                 -1);
    } else if (i == 3) {
      cv::circle(cv_ptr->image, board_image_pixels[i], 4, CV_RGB(255, 255, 0),
                 -1);
    } else if (i == 4) {
      cv::circle(cv_ptr->image, board_image_pixels[i], 4, CV_RGB(0, 255, 255),
                 -1);
    }
  }
  ...
  double pixdiagonal = sqrt(
      pow(inner_cbcorner_pixels.front().x - inner_cbcorner_pixels.back().x, 2) +
      (pow(inner_cbcorner_pixels.front().y - inner_cbcorner_pixels.back().y,
           2)));
  double len_diagonal =
      sqrt(pow(corners_3d.front().x - corners_3d.back().x, 2) +
...
FeatureExtractor::locateChessboard(const sensor_msgs::Image::ConstPtr& image) {
  ...
  bool pattern_found = findChessboardCorners(
      gray, i_params.chessboard_pattern_size, cornersf,
  ...
  cv::Mat z = cv::Mat(cv::Point3d(
      0., 0., -1.));  // TODO: why is this normal -1 in z? Surabhi's is just 1
...
FeatureExtractor::extractBoard(const PointCloud::Ptr& cloud,
  ...
  cv::Point3d lidar_normal(coefficients->values[0], coefficients->values[1],
...
FeatureExtractor::findEdges(const PointCloud::Ptr& edge_pair_cloud) {
  ...
  pcl::PointIndices::Ptr full_inliers(new pcl::PointIndices),
      half_inliers(new pcl::PointIndices);
...
void FeatureExtractor::distoffset_passthrough(
    const PointCloud::ConstPtr& input_pc, PointCloud::Ptr& output_pc) {
  if (distance_offset != 0) {
    ...
    for (auto p : input_pc->points) {
      ...
      float theta =
          acosf(z / r);  // inclination (vertical) - angle between positive
                         // z-axis and line from origin to point
      float phi =
          atanf(y / x);  // azimuth (horizontal) - angle between positive x-axis
      ...
      if (octant_before == 2 || octant_before == 6 || octant_before == 3 ||
          octant_before == 7) {
        phi = M_PI + phi;
      }
...
void FeatureExtractor::extractRegionOfInterest(
  ...
  if (background_pc_samples_.size() == 1) {
    ...
    pcl::octree::OctreePointCloudChangeDetector<pcl::PointXYZIR> octree(
        bounds_.voxel_res);
  ...
  if (flag == Optimise::Request::CAPTURE) {
    ...
    if (num_of_pc_frames_ == frames_to_capture_) {
      ...
      } else  // For every other sample get the dimensions and compare to the
              // first one
      {
        ...
        cv::Point3d avg_camera_c0 =
            cv::Point3d(sum_camera_c0.x / samples_.size(),
                        sum_camera_c0.y / samples_.size(),
                        sum_camera_c0.z / samples_.size());
        cv::Point3d avg_camera_c1 =
            cv::Point3d(sum_camera_c0.x / samples_.size(),
                        sum_camera_c0.y / samples_.size(),
                        sum_camera_c0.z / samples_.size());
        cv::Point3d avg_camera_c2 =
            cv::Point3d(sum_camera_c0.x / samples_.size(),
                        sum_camera_c0.y / samples_.size(),
                        sum_camera_c0.z / samples_.size());
        cv::Point3d avg_camera_c3 =
            cv::Point3d(sum_camera_c0.x / samples_.size(),
                        sum_camera_c0.y / samples_.size(),
                        sum_camera_c0.z / samples_.size());

        cv::Point3d avg_lidar_c0 = cv::Point3d(
            sum_lidar_c0.x / samples_.size(), sum_lidar_c0.y / samples_.size(),
            sum_lidar_c0.z / samples_.size());

        cv::Point3d avg_lidar_c1 = cv::Point3d(
            sum_lidar_c1.x / samples_.size(), sum_lidar_c1.y / samples_.size(),
            sum_lidar_c1.z / samples_.size());

        cv::Point3d avg_lidar_c2 = cv::Point3d(
            sum_lidar_c2.x / samples_.size(), sum_lidar_c2.y / samples_.size(),
            sum_lidar_c2.z / samples_.size());

        cv::Point3d avg_lidar_c3 = cv::Point3d(
            sum_lidar_c3.x / samples_.size(), sum_lidar_c3.y / samples_.size(),
            sum_lidar_c3.z / samples_.size());

        double gt_area =
            (double)board_width_ / 1000 * (double)board_height_ / 1000;
        double b_area = (avg_w0 / 1000 * avg_h0 / 1000) / 2 +
                        (avg_w1 / 1000 * avg_h1 / 1000) / 2;
        ...
        double distance = sqrt(pow(sample.lidar_centre.x / 1000 - 0, 2) +
                               pow(sample.lidar_centre.y / 1000 - 0, 2) +
                               pow(sample.lidar_centre.z / 1000 - 0, 2));

        sample.camera_centre =
            cv::Point3d(sum_camera_centre.x / samples_.size(),
                        sum_camera_centre.y / samples_.size(),
                        sum_camera_centre.z / samples_.size());

        sample.camera_normal =
            cv::Point3d(sum_camera_normal.x / samples_.size(),
                        sum_camera_normal.y / samples_.size(),
                        sum_camera_normal.z / samples_.size());

        sample.camera_corners = {avg_camera_c0, avg_camera_c1, avg_camera_c2,
                                 avg_camera_c3};

        sample.lidar_centre = cv::Point3d(sum_lidar_centre.x / samples_.size(),
                                          sum_lidar_centre.y / samples_.size(),
                                          sum_lidar_centre.z / samples_.size());

        sample.lidar_normal = cv::Point3d(sum_lidar_normal.x / samples_.size(),
                                          sum_lidar_normal.y / samples_.size(),
                                          sum_lidar_normal.z / samples_.size());

        sample.lidar_corners = {avg_lidar_c0, avg_lidar_c1, avg_lidar_c2,
                                avg_lidar_c3};
        ...
        printf("\n--- Sample %d ---\n", num_samples);
        printf(
            "Measured board has: dimensions = %6.5f x %6.5f mm; area = %6.5f "
            "m^2\n",
        ...
        printf("Board angles     = %5.2f,%5.2f,%5.2f,%5.2f degrees\n",
               sample.angles[0], sample.angles[1], sample.angles[2],
               sample.angles[3]);
        printf("Board area       = %7.5f m^2 (%+4.5f m^2)\n", b_area,
               b_area - gt_area);
        printf("Board avg height = %6.2fmm (%+4.2fmm)\n", (avg_h0 + avg_h1) / 2,
               (avg_h0 + avg_h1) / 2 - board_height_);
        printf("Board avg width  = %6.2fmm (%+4.2fmm)\n", (avg_w0 + avg_w1) / 2,
               (avg_w0 + avg_w1) / 2 - board_width_);
        printf("Board dim        = %6.2f,%6.2f,%6.2f,%6.2f mm\n", avg_w0,
               avg_h0, avg_h1, avg_w1);
        printf("Board dim error  = %7.2f\n\n", be_dim_err);

        // If the lidar board dim is more than 10% of the measured, then reject
        // sample
        if ((abs(avg_w0 - board_width_) > board_width_ * 0.1) |
            (abs(avg_w1 - board_width_) > board_width_ * 0.1) |
            (abs(avg_h0 - board_height_) > board_height_ * 0.1) |
            (abs(avg_h1 - board_height_) > board_height_ * 0.1)) {
          ROS_ERROR(
              "Plane fitting error, LiDAR board dimensions incorrect; "
              "discarding sample - try capturing again");
        ...
        std::string img_filepath = newdatafolder + "/images/pose" +
                                   std::to_string(num_samples) + ".png";
        std::string target_pcd_filepath = newdatafolder + "/pcd/pose" +
                                          std::to_string(num_samples) +
                                          "_target.pcd";
        std::string full_pcd_filepath = newdatafolder + "/pcd/pose" +
                                        std::to_string(num_samples) +
                                        "_full.pcd";
        ...
        if (num_samples == 1) {
          ...
          camera_info_file.open(camera_info_path,
                                std::ios_base::out | std::ios_base::trunc);
          std::string dist_model =
              (i_params.fisheye_model) ? "fisheye" : "non-fisheye";
          ...
          camera_info_file << "K: [" << i_params.cameramat.at<double>(0, 0)
                           ...
                           << i_params.cameramat.at<double>(1, 2) << ",0.0,0.0"
      ...
      if (corner_vectors.size() == 0) {
        flag = Optimise::Request::READY;
        ROS_ERROR(
            "Sample capture failed: can't detect chessboard in camera image");
        ROS_INFO("Ready to capture sample");
        return;
      }
      ...
      auto [cloud_projected, lidar_normal] =
          extractBoard(cloud_filtered, sample);
      ...
      for (auto& ring : ring_pointclouds) {
        std::sort(
            ring.begin(), ring.end(),
            [](pcl::PointXYZIR p1, pcl::PointXYZIR p2) { return p1.y > p2.y; });
      }
      ...
      if (top_left.values.empty() | top_right.values.empty() |
        ...
        ROS_ERROR(
            "RANSAC unsuccessful, discarding sample - Need more lidar points "
            "on board");
        flag = Optimise::Request::READY;
        ROS_INFO("Ready for capture\n");
        return;
      ...
      cv::Mat top_left_vector = (cv::Mat_<double>(3, 1) << top_left.values[3],
                                 top_left.values[4], top_left.values[5]);
      cv::Mat top_right_vector = (cv::Mat_<double>(3, 1) << top_right.values[3],
                                  top_right.values[4], top_right.values[5]);
      cv::Mat bottom_left_vector =
          (cv::Mat_<double>(3, 1) << bottom_left.values[3],
           bottom_left.values[4], bottom_left.values[5]);
      cv::Mat bottom_right_vector =
          (cv::Mat_<double>(3, 1) << bottom_right.values[3],
           bottom_right.values[4], bottom_right.values[5]);
      double a0 = acos(top_left_vector.dot(top_right_vector)) * 180 / M_PI;
      double a1 =
          acos(bottom_left_vector.dot(bottom_right_vector)) * 180 / M_PI;
      ...
      double top_down_radius =
          sqrt(pow(sample.lidar_centre.x, 2) + pow(sample.lidar_centre.y, 2));
      double vector_dist =
          sqrt(pow(sample.lidar_centre.x + sample.lidar_normal.x, 2) +
      ...
      lengths.push_back(sqrt(pow(c0.x - c3.x, 2) + pow(c0.y - c3.y, 2) +
                             pow(c0.z - c3.z, 2)) *
                        1000);
      lengths.push_back(sqrt(pow(c0.x - c2.x, 2) + pow(c0.y - c2.y, 2) +
                             pow(c0.z - c2.z, 2)) *
                        1000);
      lengths.push_back(sqrt(pow(c1.x - c3.x, 2) + pow(c1.y - c3.y, 2) +
                             pow(c1.z - c3.z, 2)) *
                        1000);
      lengths.push_back(sqrt(pow(c1.x - c2.x, 2) + pow(c1.y - c2.y, 2) +
                             pow(c1.z - c2.z, 2)) *
                        1000);
      ...
      pc_samples_set_.push_back(cloud_projected);
      publishBoardPointCloud();

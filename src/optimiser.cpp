...
void Optimiser::init_genes(RotationTranslation& p,
                           const std::function<double(void)>& rnd01,
                           const RotationTranslation& initial_rot_trans,
...
double Optimiser::perpendicularCost(const Rotation& rot) {
  ...
  for (const auto& sample : current_set_) {
    ...
    auto perp =
        cv::Mat(sample.lidar_centre - sample.lidar_corners[0]).reshape(1);
    perp /= cv::norm(perp);
    cost +=
        std::pow(perp.dot(camera_normal_lidar_frame), 2) / current_set_.size();
  }
  return cost;
}
...
double Optimiser::normalAlignmentCost(const Rotation& rot) {
  ...
  for (const auto& sample : current_set_) {
    auto camera_normal_lidar_frame = rot * sample.camera_normal;
    cost += cv::norm(camera_normal_lidar_frame -
                     cv::Mat(sample.lidar_normal).reshape(1)) /
            current_set_.size();
  }
  return cost;
}
...
double Optimiser::reprojectionCost(const RotationTranslation& rot_trans) {
  ...
  for (auto& sample : current_set_) {
    ...
    auto camera_centre_lidar_frame =
        cv::Point3d(rot_trans * sample.camera_centre);
    ...
    if (i_params_.fisheye_model) {
      cv::fisheye::projectPoints(cam_centre_3d, cam, rvec, tvec,
                                 i_params_.cameramat, i_params_.distcoeff);
      cv::fisheye::projectPoints(lidar_centre_3d, lidar, rvec, tvec,
                                 i_params_.cameramat, i_params_.distcoeff);
    } else {
      cv::projectPoints(cam_centre_3d, rvec, tvec, i_params_.cameramat,
                        i_params_.distcoeff, cam);
      cv::projectPoints(lidar_centre_3d, rvec, tvec, i_params_.cameramat,
                        i_params_.distcoeff, lidar);
    }
...
double Optimiser::centreAlignmentCost(const RotationTranslation& rot_trans) {
  ...
  for (const auto& sample : current_set_) {
    ...
    abs_mean += cv::norm(camera_centre_lidar_frame -
                         cv::Mat(sample.lidar_centre).reshape(1)) /
                current_set_.size();
  }
  double stddev = 0;
  for (const auto& sample : current_set_) {
    auto camera_centre_lidar_frame = rot_trans * sample.camera_centre;
    stddev += std::pow(cv::norm(camera_centre_lidar_frame -
                                cv::Mat(sample.lidar_centre).reshape(1)) -
                           abs_mean,
                       2) /
              current_set_.size();
...
bool Optimiser::eval_solution(const RotationTranslation& p,
                              RotationTranslationCost& c) {
  ...
  c.objective2 =
      perpendicular_cost + normal_align_cost + centre_align_cost + repro_cost;

  return true;  // solution is accepted
}

RotationTranslation Optimiser::mutate(
  ...
  do {
    ...
    in_range = in_range &&
               (X_new.rot.roll >=
                    (initial_rotation_translation.rot.roll - angle_increment) &&
                X_new.rot.roll <
                    (initial_rotation_translation.rot.roll + angle_increment));
    float pitch_inc = 0.2 * (rnd01() - rnd01()) * shrink_scale;
    X_new.rot.pitch += pitch_inc;
    in_range = in_range &&
               (X_new.rot.pitch >= (initial_rotation_translation.rot.pitch -
                                    angle_increment) &&
                X_new.rot.pitch <
                    (initial_rotation_translation.rot.pitch + angle_increment));
    float yaw_inc = 0.2 * (rnd01() - rnd01()) * shrink_scale;
    X_new.rot.yaw += yaw_inc;
    in_range = in_range &&
               (X_new.rot.yaw >=
                    (initial_rotation_translation.rot.yaw - angle_increment) &&
                X_new.rot.yaw <
                    (initial_rotation_translation.rot.yaw + angle_increment));
...
RotationTranslation Optimiser::crossover(
    const RotationTranslation& X1, const RotationTranslation& X2,
    const std::function<double(void)>& rnd01) {
  RotationTranslation X_new;
  double r;
  r = rnd01();
...
double Optimiser::calculate_SO_total_fitness(
    const GA_Rot_Trans_t::thisChromosomeType& X) {
  // finalize the cost
...
void Optimiser::SO_report_generation(
    ...
    const EA::GenerationType<RotationTranslation, RotationTranslationCost>&
        last_generation,
...
void Optimiser::init_genes(Rotation& p,
                           const std::function<double(void)>& rnd01,
                           const Rotation& initial_rotation, double increment) {
...
Rotation Optimiser::mutate(const Rotation& X_base,
                           const std::function<double(void)>& rnd01,
                           const Rotation& initial_rotation,
                           const double angle_increment, double shrink_scale) {
  Rotation X_new;
  bool in_range;
  do {
    in_range = true;
    ...
    X_new.roll += roll_inc;
    in_range =
        in_range && (X_new.roll >= (initial_rotation.roll - angle_increment) &&
    ...
    X_new.yaw += yaw_inc;
    in_range =
        in_range && (X_new.yaw >= (initial_rotation.yaw - angle_increment) &&
                     X_new.yaw < (initial_rotation.yaw + angle_increment));
  } while (!in_range);
  return X_new;
}

Rotation Optimiser::crossover(const Rotation& X1, const Rotation& X2,
                              const std::function<double(void)>& rnd01) {
  Rotation X_new;
  double r = rnd01();
...
double Optimiser::calculate_SO_total_fitness(
    const GA_Rot_t::thisChromosomeType& X) {
...
void Optimiser::get_mean_stdev(std::vector<float>& input_vec, float& mean,
                               float& stdev) {
...
void Optimiser::generate_sets(int offset, int k,
  ...
  for (std::size_t i = offset; i <= samples.size() - (std::size_t)k; ++i) {
...
bool Optimiser::optimise(RotationTranslation& opt_result,
                         std::vector<OptimisationSample>& set,
  ...
  for (auto& sample : current_set_) {
    float dim_sum = abs(sample.widths[0] - i_params_.board_dimensions.width) +
  ...
  cv::Mat UNR =
      (NN.inv() * NM).t();  // Analytical rotation matrix for real data
  ...
  float b_avg = std::accumulate(std::begin(b_dims), std::end(b_dims), 0.0) /
                b_dims.size();
  ...
  ga_obj.calculate_SO_total_fitness =
      [&](const GA_Rot_t::thisChromosomeType& X) -> double {
    return this->calculate_SO_total_fitness(X);
  };
  ga_obj.init_genes = [&, initial_rotation, rotation_increment](
                          Rotation& p,
                          const std::function<double(void)>& rnd01) -> void {
    this->init_genes(p, rnd01, initial_rotation, rotation_increment);
  };
  ga_obj.eval_solution = [&](const Rotation& r, RotationCost& c) -> bool {
    return this->eval_solution(r, c);
  };
  ga_obj.mutate = [&, initial_rotation, rotation_increment](
                      const Rotation& X_base,
                      const std::function<double(void)>& rnd01,
                      double shrink_scale) -> Rotation {
    return this->mutate(X_base, rnd01, initial_rotation, rotation_increment,
                        shrink_scale);
  };
  ga_obj.crossover = [&](const Rotation& X1, const Rotation& X2,
                         const std::function<double(void)>& rnd01) {
  ...
  cv::Mat tmp_rot =
      (cv::Mat_<double>(3, 3) << rot.getRow(0)[0], rot.getRow(0)[1],
       rot.getRow(0)[2], rot.getRow(1)[0], rot.getRow(1)[1], rot.getRow(1)[2],
  ...
  const RotationTranslation initial_rotation_translation{
      best_rotation_, summed_diff.at<double>(0), summed_diff.at<double>(1),
      summed_diff.at<double>(2)};
  ...
  // Joint optimization for Rotation and Translation (Perform this 10 times and
  // take the average of the extrinsics)
  ...
  ga_rot_trans.calculate_SO_total_fitness =
      [&](const GA_Rot_Trans_t::thisChromosomeType& X) -> double {
    return this->calculate_SO_total_fitness(X);
  };
  ga_rot_trans.init_genes =
      [&, initial_rotation_translation, rotation_increment,
       translation_increment](
          RotationTranslation& p,
          const std::function<double(void)>& rnd01) -> void {
    this->init_genes(p, rnd01, initial_rotation_translation, rotation_increment,
                     translation_increment);
  };
  ga_rot_trans.eval_solution = [&](const RotationTranslation& rt,
                                   RotationTranslationCost& c) -> bool {
    return this->eval_solution(rt, c);
  };
  ga_rot_trans.mutate = [&, initial_rotation_translation, rotation_increment,
                         translation_increment](
                            const RotationTranslation& X_base,
                            const std::function<double(void)>& rnd01,
                            double shrink_scale) -> RotationTranslation {
    return this->mutate(X_base, rnd01, initial_rotation_translation,
                        rotation_increment, translation_increment,
                        shrink_scale);
  };
  ga_rot_trans.crossover = [&](const RotationTranslation& X1,
                               const RotationTranslation& X2,
                               const std::function<double(void)>& rnd01) {
    return this->crossover(X1, X2, rnd01);
  };
  ga_rot_trans.SO_report_generation =
      [&](int generation_number,
          const EA::GenerationType<RotationTranslation,
                                   RotationTranslationCost>& last_generation,
  ...
  printf("| % 05.3f,% 05.3f,% 05.3f,% 05.3f,% 05.3f,% 05.3f ",
         opt_result.rot.roll, opt_result.rot.pitch, opt_result.rot.yaw,
...
std::vector<double> rotm2eul(cv::Mat mat) {
  ...
  euler[0] = atan2(mat.at<double>(2, 1),
                   mat.at<double>(2, 2));  // rotation about x axis: roll
  euler[1] = atan2(-mat.at<double>(2, 0),
                   sqrt(mat.at<double>(2, 1) * mat.at<double>(2, 1) +
                        mat.at<double>(2, 2) * mat.at<double>(2, 2)));
  euler[2] = atan2(mat.at<double>(1, 0),
                   mat.at<double>(0, 0));  // rotation about z axis: yaw
  return euler;
}
}  // namespace cam_lidar_calibration

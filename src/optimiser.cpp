...
void Optimiser::init_genes(
  RotationTranslation & p, const std::function<double(void)> & rnd01,
...
double Optimiser::normalAlignmentCost(const Rotation & rot)
{
  ...
  for (const auto & sample : current_set_) {
    auto camera_normal_lidar_frame = rot * sample.camera_normal;
    cost += cv::norm(camera_normal_lidar_frame - cv::Mat(sample.lidar_normal).reshape(1)) /
            current_set_.size();
  }
  return cost;
}
  ...
  for (auto & sample : current_set_) {
    ...
    if (i_params_.fisheye_model) {
      cv::fisheye::projectPoints(
        cam_centre_3d, cam, rvec, tvec, i_params_.cameramat, i_params_.distcoeff);
      cv::fisheye::projectPoints(
        lidar_centre_3d, lidar, rvec, tvec, i_params_.cameramat, i_params_.distcoeff);
    } else {
      cv::projectPoints(cam_centre_3d, rvec, tvec, i_params_.cameramat, i_params_.distcoeff, cam);
      cv::projectPoints(
        lidar_centre_3d, rvec, tvec, i_params_.cameramat, i_params_.distcoeff, lidar);
    }
...
double Optimiser::centreAlignmentCost(const RotationTranslation & rot_trans)
{
  ...
  for (const auto & sample : current_set_) {
    ...
    abs_mean += cv::norm(camera_centre_lidar_frame - cv::Mat(sample.lidar_centre).reshape(1)) /
                current_set_.size();
  }
  double stddev = 0;
  for (const auto & sample : current_set_) {
    ...
    stddev +=
      std::pow(
        cv::norm(camera_centre_lidar_frame - cv::Mat(sample.lidar_centre).reshape(1)) - abs_mean,
        2) /
      current_set_.size();
  }
  ...
  const double translation_increment, const double shrink_scale)
{
  ...
  do {
    ...
    in_range =
      in_range && (X_new.rot.roll >= (initial_rotation_translation.rot.roll - angle_increment) &&
    ...
    in_range =
      in_range && (X_new.rot.pitch >= (initial_rotation_translation.rot.pitch - angle_increment) &&
    ...
    in_range =
      in_range && (X_new.rot.yaw >= (initial_rotation_translation.rot.yaw - angle_increment) &&
...
void Optimiser::init_genes(
  Rotation & p, const std::function<double(void)> & rnd01, const Rotation & initial_rotation,
  double increment)
{
...
Rotation Optimiser::mutate(
  const Rotation & X_base, const std::function<double(void)> & rnd01,
...
Rotation Optimiser::crossover(
  const Rotation & X1, const Rotation & X2, const std::function<double(void)> & rnd01)
{
  Rotation X_new;
  double r = rnd01();
...
// A function to show/store the results of each generation.
void Optimiser::SO_report_generation(
  int generation_number, const EA::GenerationType<Rotation, RotationCost> & last_generation,
...
void Optimiser::generate_sets(
  int offset, int k, std::vector<OptimisationSample> & set,
...
bool Optimiser::optimise(
  RotationTranslation & opt_result, std::vector<OptimisationSample> & set, cv::Mat & cameramat,
  cv::Mat & distcoeff)
{
  ...
  ga_obj.init_genes = [&, initial_rotation, rotation_increment](
                        Rotation & p, const std::function<double(void)> & rnd01) -> void {
  ...
  cv::Mat tmp_rot =
    (cv::Mat_<double>(3, 3) << rot.getRow(0)[0], rot.getRow(0)[1], rot.getRow(0)[2],
     rot.getRow(1)[0], rot.getRow(1)[1], rot.getRow(1)[2], rot.getRow(2)[0], rot.getRow(2)[1],
     rot.getRow(2)[2]);
  ...
  ga_rot_trans.init_genes =
    [&, initial_rotation_translation, rotation_increment, translation_increment](
      RotationTranslation & p, const std::function<double(void)> & rnd01) -> void {
    this->init_genes(
      p, rnd01, initial_rotation_translation, rotation_increment, translation_increment);
  };
  ga_rot_trans.eval_solution =
    [&](const RotationTranslation & rt, RotationTranslationCost & c) -> bool {
  ...
  ga_rot_trans.mutate =
    ...
    return this->mutate(
      X_base, rnd01, initial_rotation_translation, rotation_increment, translation_increment,
      shrink_scale);
  };
  ga_rot_trans.crossover = [&](
                             const RotationTranslation & X1, const RotationTranslation & X2,
                             const std::function<double(void)> & rnd01) {
    return this->crossover(X1, X2, rnd01);
  };
  ...
  printf(
    "| % 05.3f,% 05.3f,% 05.3f,% 05.3f,% 05.3f,% 05.3f ", opt_result.rot.roll, opt_result.rot.pitch,
    opt_result.rot.yaw, opt_result.x / 1000.0, opt_result.y / 1000.0, opt_result.z / 1000.0);
  return true;
}
...
std::vector<double> rotm2eul(cv::Mat mat)
{
  ...
  euler[1] = atan2(
    -mat.at<double>(2, 0),
    sqrt(
      mat.at<double>(2, 1) * mat.at<double>(2, 1) + mat.at<double>(2, 2) * mat.at<double>(2, 2)));

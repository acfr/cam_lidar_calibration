...
{
  double objective1;  // This is where the results of simulation is stored but
                      // not yet finalized.
};

struct RotationTranslationCost  // equivalent to y in matlab
{
  double objective2;  // This is where the results of simulation is stored but
                      // not yet finalized.
};
...
struct OptimisationSample
{
  ...
  float distance_from_origin;  // Currently unused - only for print statements
                               // on capture
  double pixeltometre;
  int sample_num;
};
...
public:
  ...
  Rotation mutate(
    ...
    const Rotation & initial_rotation, const double angle_increment, const double shrink_scale);
...
private:
  ...
  std::vector<double> analytical_euler(
    std::vector<OptimisationSample> & set, cv::Mat & camera_centres_, cv::Mat & camera_normals_,
    cv::Mat & lidar_centres_, cv::Mat & lidar_normals_);
